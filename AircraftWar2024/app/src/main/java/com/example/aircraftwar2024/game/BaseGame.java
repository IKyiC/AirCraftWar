...
public abstract class BaseGame extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    protected double enemyCycle = 10;
    /**
     * 控制精英机产生概率，默认值设为简单模式
     */
    ...
    public boolean isGameOverFlag() {return gameOverFlag; }
            ...
            if (heroAircraft.crash(flyingSupply) || flyingSupply.crash(heroAircraft)) {
                if(isMusicOn) {
                    if(flyingSupply instanceof BombSupply) {
                        mySoundPool.explosionSoundPool();
                        mySoundPool.supplySoundPool();
                    }
                    else {
                        mySoundPool.supplySoundPool();
                    }
                ...
                if(flyingSupply instanceof BombSupply) {
                    for (AbstractBullet enemyBullet: enemyBullets) {
                        ((BombSupply) flyingSupply).addObserver((Observer) enemyBullet);
                    }

                    for (AbstractAircraft enemy: enemyAircrafts) {
                        ((BombSupply) flyingSupply).addObserver((Observer) enemy);
    ...
    private void paintScoreAndLife() {
        ...
        canvas.drawText("LIFE:"+this.heroAircraft.getHp(), x, y+50, mPaint);

        if(gameType == "Online") {
            canvas.drawText("ENEMY SCORE:"+ MainActivity.GetOpScore(),x+400, y, mPaint);
        }
    }
    ...
    public void run() {
        ...
        if(gameType == "Offline") {
            ...
            msg.what = 1; //信息标识
            msg.obj = userScore; //将userScore作为额外数据传输过去
